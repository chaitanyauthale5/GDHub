...
const GDTopicsGenerator = () => {
  ...
  const generateTopics = async () => {
    ...
    setError('');
    try {
      const response = await api.gdTopics.generateTopics();
      if (response.success) {
  ...
  const fetchHeadlines = async () => {
    ...
    setError('');
    try {
      const response = await api.gdTopics.getHeadlines();
      if (response.success) {
                ...
                {headlines.map((headline, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-3 hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start">
                      <h4 className="font-medium text-gray-900 flex-1">{headline.title}</h4>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {categories.map((category, index) => (
                  <div key={index} className={`px-4 py-3 rounded-lg text-center font-medium ${getCategoryColor(category)}`}>
                    {category}
                  </div>
                ))}
              </div>
